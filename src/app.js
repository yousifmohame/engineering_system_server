...
const app = express();
const zktecoController = require('../src/controllers/zktecoController');
// ==================================================
...
// يجب أن نسمح للسيرفر بقراءة النصوص الخام القادمة من جهاز البصمة
app.use('/api/iclock', express.text({ type: '*/*' })); 

// 1. مسار التهيئة (عندما يشتغل الجهاز، يتصل بهذا المسار ليقول "أنا هنا")
app.get('/api/iclock/cdata', zktecoController.handshake);

// 2. مسار استقبال البصمات (عندما يبصم الموظف، يرسل الجهاز البيانات هنا)
app.post('/api/iclock/cdata', zktecoController.receiveData);

// 3. مسار إرسال الأوامر للجهاز (اختياري)
app.get('/api/iclock/getrequest', zktecoController.getCommands);
