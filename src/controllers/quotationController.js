...
const generatePdfPreview = async (req, res) => {
  try {
    ...
    res.set({
      ...
      "Content-Disposition": `attachment; filename="${data.referenceNumber || "preview"}.pdf"`,
    });
    res.send(pdfBuffer);
  } catch (error) {
    console.error(
...
const approveQuotationWorkflow = async (req, res) => {
  ...
  try {
    ...
    // 4. تحديث الداتا بيز بحالة الاعتماد والروابط
    const updatedQuote = await prisma.$transaction(async (tx) => {
      ...
      await tx.quotationLog.create({
        data: {
          ...
          userId,
          userName,
          notes: "تم اعتماد وختم العرض وتوليد PDF",
        },
      });
      return updated;
    });
    ...
    const fileName = `QT_${quote.id}_${Date.now()}.pdf`;
    const filePath = path.join(uploadsDir, fileName);
